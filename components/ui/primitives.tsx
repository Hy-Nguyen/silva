...
export function ImageSlot({ label, ratio = '16/9', filled, src, alt = '', style }: ImageSlotProps) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: 'var(--f-body)', fontSize: 11, fontWeight: 500, letterSpacing: '0.02em', textTransform: 'uppercase', opacity: 0.7 }}>
        {label}
      </div>
      <div style={{ fontFamily: 'var(--f-body)', fontSize: 10, opacity: 0.5, fontVariantNumeric: 'tabular-nums' }}>{ratio}</div>
    </div>
  )
}
    ...
    <div style={{ display: 'inline-flex', gap: 2, color: 'var(--c-accent)' }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          ...
          size={size}
          strokeWidth={1.5}
          style={{ fill: i < rating ? 'currentColor' : 'transparent' }}
        />
      ))}
    </div>
  )
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, ...style }}>
        <div
          style={{
            ...
            background: 'repeating-linear-gradient(135deg, var(--c-surface-2), var(--c-surface-2) 6px, var(--c-border) 6px, var(--c-border) 7px)',
            border: '1px dashed var(--c-border)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
