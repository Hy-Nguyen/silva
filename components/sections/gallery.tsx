...
export function Gallery({ data, filled }: GalleryProps) {
	...
	const next = () => setLightbox((p) => (p !== null ? (p + 1) % data.images.gallery.length : null));
	const prev = () =>
		setLightbox((p) => (p !== null ? (p - 1 + data.images.gallery.length) % data.images.gallery.length : null));

	return (
		<section
			id='gallery'
			style={{ background: "var(--c-surface-2)", padding: "clamp(72px, 8vw, 120px) clamp(24px, 5vw, 64px)" }}
		>
			<div style={{ maxWidth: 1280, margin: "0 auto" }}>
				...
				>
					<motion.div variants={reduced ? undefined : stagger()} initial='hidden' whileInView='show' viewport={VP}>
				...
				<motion.div
					...
					whileInView='show'
					viewport={VP}
					style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "clamp(10px, 1.5vw, 16px)" }}
